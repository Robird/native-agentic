...
- 增补“自写训练批注”原语：不只写 MemoryNotebook，还把值得沉淀的经验、判断和决心写成可筛选、可蒸馏的未来训练语料候选。
- 增补树状探索与回溯原语：让 agent 能显式记录分叉点、探索一个分支、带结果回主线，并把不互斥的选项挂入后续目标树。
- 明确采用分阶段后训练假设：先把主体性、判断与长期性教对，再把工具调用、结构化执行和 RLVR 式能力强化补齐。
...
- 评估“先教做人，再教做事”的两阶段训练配方是否比一步到位更稳。

交付物：

- 训练序列打包器。
- 消融方案：只用分析教师、分析+扮演、分析+扮演+故事、是否保留质量控制字段等。
- 两阶段训练草案：阶段 A 先学主体性、长期目标和判断姿态，阶段 B 再学工具调用、环境操作和执行强化。
